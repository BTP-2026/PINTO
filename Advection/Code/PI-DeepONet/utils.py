...
def get_train_data(data_dir, sensor_samples, domain_samples, indices, val_indices):

    sd = h5py.File(data_dir, 'r')
    data = sd['tensor'][:]
    xdisc = sd['x-coordinate'][:]
    tdisc = sd['t-coordinate'][:-1]
    ...
    xbc_in = np.repeat(x_sensor, ins, axis=1).reshape((-1, sensor_samples))
    tbc_in = np.repeat(t_sensor, ins, axis=1).reshape((-1, sensor_samples))
    ubc_in = np.repeat(u_sensor, ins, axis=1).reshape((-1, sensor_samples))

    xbc_b = np.repeat(x_sensor, bs, axis=1).reshape((-1, sensor_samples))
    tbc_b = np.repeat(t_sensor, bs, axis=1).reshape((-1, sensor_samples))
    ubc_b = np.repeat(u_sensor, bs, axis=1).reshape((-1, sensor_samples))
